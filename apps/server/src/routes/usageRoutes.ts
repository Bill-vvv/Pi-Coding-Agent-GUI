...
import { emptyTokenUsageOverview, normalizeTokenUsageRange, TokenUsageService } from "../services/tokenUsageService.js";
...
export async function registerUsageRoutes(fastify: FastifyInstance, { db, service = new TokenUsageService() }: UsageRouteOptions): Promise<void> {
  fastify.get("/api/usage/overview", async (request) => {
    ...
    try {
      ...
      request.log.warn({ err: error }, "Token usage overview failed; returning empty overview");
      return { usage: emptyTokenUsageOverview(range, projectId) };
    }
  });
}
